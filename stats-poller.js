...
const postToDiscord = async (instance, race, raceDetails, member) => {
    ...
    const formatLicense = (raceDetails) => {
      if (raceDetails.race.new_license_level <= 4) {
        return `R ${raceDetails.race.new_sub_level / 100.0} (${safetyFormatter.format((raceDetails.race.new_sub_level - raceDetails.race.old_sub_level) / 100.0)})`;
      }
      if (raceDetails.race.new_license_level <= 8) {
        return `D ${raceDetails.race.new_sub_level / 100.0} (${safetyFormatter.format((raceDetails.race.new_sub_level - raceDetails.race.old_sub_level) / 100.0)})`;
      }
      if (raceDetails.race.new_license_level <= 12) {
        return `C ${raceDetails.race.new_sub_level / 100.0} (${safetyFormatter.format((raceDetails.race.new_sub_level - raceDetails.race.old_sub_level) / 100.0)})`;
      }
      if (raceDetails.race.new_license_level <= 16) {
        return `B ${raceDetails.race.new_sub_level / 100.0} (${safetyFormatter.format((raceDetails.race.new_sub_level - raceDetails.race.old_sub_level) / 100.0)})`;
      }
      if (raceDetails.race.new_license_level <= 20) {
        return `A ${raceDetails.race.new_sub_level / 100.0} (${safetyFormatter.format((raceDetails.race.new_sub_level - raceDetails.race.old_sub_level) / 100.0)})`;
      }
...
redis.connect().then(async () => {
  ...
  for (const member of roster) {
    ...
    if (races.length > 0) {
      const latestRace = races.sort((a, b) => new Date(b.end_time).getTime() - new Date(a.end_time).getTime())?.[0];
