...
async function getSession(instance, subSessionId) {
  ...
  return data.data.session_results[0].results.map(d => {
    return {
      cust_id: d.cust_id,
      subsession_id: subSessionId,
...
const getSessions = async () => {
  ...
  const times = await Promise.all(sessions.map(async user => {
    //console.log(user)
    ...
    if (data.data.chunk_info?.base_download_url) {
      ...
      console.log(user.display_name, validLapTimes.filter(t => t > 0).map(t => new Date(t)).map(t => t.getMinutes() + ":" + t.getSeconds() + ":" + t.getMilliseconds()));
